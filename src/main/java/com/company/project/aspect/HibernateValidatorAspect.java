...
import com.company.common.exception.IllegalValidateException;
import com.company.common.request.RestStatus;
import com.company.common.response.ErrorEntity;
...
public class HibernateValidatorAspect implements Ordered {
    ...
    @Around(value = "within(com.company.project..*)&&(@annotation(org.springframework.web.bind.annotation.PostMapping)|| @annotation(org.springframework.web.bind.annotation.RequestMapping))")
    ...
    public static void throwIfInvalidModel(BindingResult result, RestStatus errorStatus) {
        ...
        if (result.getErrorCount() > 0) {
            ...
            for (FieldError fieldError : result.getFieldErrors()) {
                ...
                LOGGER.debug(" error field: {}, error msg: {}",
    ...
    private static String acquireFieldName(BindingResult result, FieldError fieldError) {
        ...
        try {
            ...
            LOGGER.error("反射字段名时抛出异常: {}", e.getMessage());
        }
